...
### C8. Consistency is a payout/pass eligibility gate, not a failure — which dissolves the continuous-check degeneracy  **[RESOLVED — model corrected]**
Earlier drafts modeled consistency as a `FAIL` rule (`max_day_pnl > threshold × total_pnl` → terminate). That was a **category error**: real firms (Lucid Pro/Flex, Topstep, and others) never *fail* an account for consistency — they *withhold a positive event*. On funded accounts, `largest_day_profit / account_profit ≤ threshold` (e.g. 40%) is required to **request a payout**; if violated the payout simply doesn't fire and the trader keeps earning on other days until the ratio falls (it resets after each payout). On eval accounts, the same ratio (e.g. 50%) is required to **upgrade/pass**. So consistency is a **conjunct in the PAYOUT predicate (funded) or PASS predicate (eval)**, reusing the fire-gate machinery of §6b — modeled as `ConsistencyGate` (ARCHITECTURE §5).

This correction *dissolves* the degeneracy that the fail-model created (and that a prior draft tried to patch with EOD-timing + an `activate_above` floor). Because the gate is only ever evaluated as part of a payout/pass conjunction — which already requires meaningful positive profit (`min_request`, the profit target) — the "total_pnl ≤ 0 → ratio undefined" and "fires on trade 1 after one good day" failures **cannot arise**: there is no scenario where consistency is tested against zero/negative profit, and violating it never terminates anything. No activation gate and no new mechanic: it reads `MAX_DAY_PNL` and **cycle profit** (`EQUITY - CYCLE_START_EQUITY`), not lifetime `TOTAL_PNL` — because the funded gate **resets after each payout**, so "account profit" means profit *this cycle*, which also makes the gate agree with the §6b payout-amount arithmetic (`cap_fraction * cycle_profit`) it sits beside. On eval this equals phase profit, since the cycle spans the whole phase when no payout has occurred. Its `check_timing` is **inert** — the gate is only read inside the payout/pass conjunction (evaluated at day close, §C9), so EOD vs CONTINUOUS are behaviorally identical and nothing may branch on it. The lesson worth keeping: the degeneracy was a *symptom of the wrong action category*, and fixing the category removed it at the root rather than patching it. `FAIL_CONSISTENCY` is reserved-but-unused unless a genuinely fail-on-consistency firm ever appears. The Flex **cushion** (effective bound slightly looser than nominal, computed on actual daily profit) is a per-firm calibration parameter on the gate, not a new mechanic.
