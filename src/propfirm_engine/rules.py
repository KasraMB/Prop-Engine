...
class ConsistencyRule(Rule):
    """No single day may exceed ``threshold`` of total profit — else the account
    fails (§5, MODEL_RISKS §C8).

    Consistency is a *whole-day* property, so it is EOD-timed by default: a
    continuous check is degenerate (``total_pnl`` is meaningless early, then
    ~1.0 after one good day, which would spuriously kill the account on day 2).
    ``activate_above`` is the profit floor below which the rule does not evaluate
    at all — the activation gate real firms use. Both the EOD timing and the gate
    are required; either alone leaves a spurious-failure path (MODEL_RISKS §C8)."""

    threshold: float
    severity: Severity = Severity.HARD
    check_timing: Timing = Timing.EOD
    activate_above: float = 0.0

    def requirements(self) -> tuple[StateField, ...]:
        return (StateField.DAY_PNL, StateField.TOTAL_PNL, StateField.MAX_DAY_PNL)

    def compile(self) -> CompiledRule:
        return CompiledRule(
            kind=RuleKind.CONSISTENCY,
            p0=self.threshold,
            p1=self.activate_above,
            action=Action.FAIL,
            severity=self.severity,
            check_timing=self.check_timing,
...
class ConsistencyRaisesTargetRule(Rule):
    ...
    kind (not a parameter of :class:`ConsistencyRule`) so the config picks the
    behaviour — fail vs adjust — explicitly. The fire-action rewrites
