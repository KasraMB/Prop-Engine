...
- Each rule reports requirements consistent with its behavior (e.g. a trailing-drawdown rule requires the equity and drawdown-floor state; a consistency rule requires the day/total/max-day state).
...
- A rule carries its two timing fields; a rule for which timing is irrelevant defaults both to continuous.
...
- **The closing day's own pnl is included in its own EOD predicate** (`_close_day` folds the day into counters *before* evaluating EOD rules, `MODEL_RISKS.md` §C9): a day that completes the 5th qualifying win pays out on that same close; a day whose own pnl tips the consistency ratio fails on that same close.
- **Consistency (FAIL variant) is EOD-timed and activation-gated** (`MODEL_RISKS.md` §C8): it does not fire intraday, and does not fire at all while `total_pnl ≤ activate_above` — so a single early good day cannot spuriously kill the account on day 2. Above the gate, it evaluates only at day close.
