...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ConsistencyRule(0.4), ConsistencyRule(0.5)),
        (
...
def test_identical_rules_are_equal_and_hash_equal():
    assert ConsistencyRule(0.5) == ConsistencyRule(0.5)
    assert hash(ConsistencyRule(0.5)) == hash(ConsistencyRule(0.5))
...
def test_consistency_requires_day_total_and_maxday_state():
    reqs = ConsistencyRule(0.5).requirements()
    assert StateField.DAY_PNL in reqs
    assert StateField.TOTAL_PNL in reqs
...
def test_fail_rules_carry_the_right_action_and_fail_code():
    cases = {
        ...
        ConsistencyRule(0.5): ExitCode.FAIL_CONSISTENCY,
    }
...
def test_consistency_defaults_to_eod_timing():
    # Consistency is a whole-day property (MODEL_RISKS §C8): EOD by default.
    assert ConsistencyRule(0.5).compile().check_timing == Timing.EOD
