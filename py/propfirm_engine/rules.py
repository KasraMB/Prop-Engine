...
class TrailingDrawdownRule(Rule):
    ...
    ``severity`` defaults to ``HARD`` (a DD breach terminates the account at every
    firm we model) but is an **overridable default**, never a hardcoded intrinsic:
    severity is a firm-varying property, so — like :class:`DailyLossRule` and
    :class:`StaticDrawdownRule` — it is exposed as a field the config can set,
    rather than baked into ``compile`` (ARCHITECTURE §5, the intrinsic-vs-firm-
    varying discipline)."""
